...
public class PlayerMov : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log("PlayerMov: Rigidbody initialized. IsKinematic=" + rb.isKinematic);
    }
    ...
    private void FixedUpdate()
    {
        ...
        // Debug: log if we have a desired movement but not moving
        bool hasDesiredMovement = fixedDesiredHorizontal.sqrMagnitude > 0.001f;
        if (hasDesiredMovement && rb.velocity.sqrMagnitude < 0.001f)
        {
            Debug.LogWarning($"PlayerMov: Has desired movement {fixedDesiredHorizontal}, but velocity is {rb.velocity}. IsKinematic={rb.isKinematic}");
        }

        // Apply horizontal desired velocity on physics step
        // For kinematic bodies, use MovePosition; for dynamic bodies, set velocity directly
        if (rb.isKinematic)
        {
            // Kinematic body: move based on desired horizontal velocity + preserve vertical velocity
            Vector3 currentVel = rb.velocity;
            Vector3 newVel = new Vector3(fixedDesiredHorizontal.x, currentVel.y, fixedDesiredHorizontal.z);
            rb.MovePosition(rb.position + newVel * Time.fixedDeltaTime);
        }
        else
        {
            // Dynamic body: set velocity directly
        ...
        if (jumpRequested)
        {
            if (rb.isKinematic)
            {
                // For kinematic bodies, manually apply jump by moving upward
                Vector3 jumpVel = Vector3.up * (pendingJumpForce / rb.mass);
                rb.MovePosition(rb.position + jumpVel * Time.fixedDeltaTime);
            }
            else
            {
                // Dynamic body: use AddForce
